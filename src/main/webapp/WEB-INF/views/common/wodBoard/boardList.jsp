...
<script>
	...
	function getBoardList() {
		...
		var url = "${contextPath }/boardWod/boardList"
			...
			<tbody id="boardList">
				<c:forEach var="boardWodDTO" items="${boardList }">
					<tr align="center">
						...
						<td>${startBoardIdx}</td>
						<td align="left">
							<a href="${contextPath }/boardWod/boardDetail?subject=${boardWodDTO.subject }">${boardWodDTO.subject }</a>
						</td>
		...
		<div style="display: table; margin-left: auto; margin-right: auto">
			<ul>
				<c:if test="${startPage > 10}">
					<li>
						<a href="${contextPath }/boardWod/boardList?currentPageNumber=${startPage - 10}&onePageViewCnt=${onePageViewCnt  }&searchKeyword=${searchKeyword }&searchWord=${searchWord}" >이전 </a>
					</li> 
				</c:if>
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<li>
						<a href="${contextPath }/boardWod/boardList?currentPageNumber=${i }&onePageViewCnt=${onePageViewCnt  }&searchKeyword=${searchKeyword }&searchWord=${searchWord}">${i } &nbsp;</a>
					</li>
				</c:forEach>
				<c:if test="${endPage != allPageCnt && endPage >= 10}">
					<li>
						<a href="${contextPath }/boardWod/boardList?currentPageNumber=${startPage + 10}&onePageViewCnt=${onePageViewCnt  }&searchKeyword=${searchKeyword }&searchWord=${searchWord}"> 다음 </a>
					</li> 
				</c:if>
			</ul>
		</div>
